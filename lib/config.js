...
function getConfig (config, swallowError = true) {
  ...
  overrideOrDefault('queryStringPasswordKeys', 'QUERY_STRING_PASSWORD_KEYS', asString, '')

  if (config.serviceName === undefined) {
    config.serviceName = 'GOV.UK Prototype Kit'
  }

  const mapPasswordKeyStringToPasswords = (keyString) => keyString.split(',')
  ...
  config.passwords = mapPasswordKeyStringToPasswords(config.passwordKeys)

  if (process.env.PASSWORD) {
    config.passwords.push(process.env.PASSWORD)
  }

  config.queryStringPasswords = mapPasswordKeyStringToPasswords(config.queryStringPasswordKeys)
      .filter(password => config.passwords.includes(password))
