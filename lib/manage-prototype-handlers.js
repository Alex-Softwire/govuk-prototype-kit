...
function processRedirectUrl (returnURL) {
  return (!returnURL || returnURL.startsWith('/manage-prototype/password')) ? '/' : returnURL
}

function setAuthenticationCookie (res, submittedPassword, cookieAgeDays) {
  res.cookie('authentication', encryptPassword(submittedPassword), {
    ...
    httpOnly: true,
    secure: config.getConfig().useHttps
  })
}

// Render password page with a returnURL to redirect people to where they came from
// Redirect to this returnURL if already authenticated or query string matches allowed password
function getPasswordHandler (req, res) {
  const returnURL = req.query.returnURL || '/'
  const error = req.query.error

  if (isAuthenticated(config.getConfig().passwords, req)) {
    res.redirect(processRedirectUrl(returnURL))
    return
  }

  const queryStringPassword = req.query.accessCode
  const validQueryStringPasswords = config.getConfig().queryStringPasswords
  if (queryStringPassword &&
      validQueryStringPasswords.some(password => queryStringPassword === password)) {
    setAuthenticationCookie(res, queryStringPassword, 1)
    res.redirect(processRedirectUrl(returnURL))
    return
  }
...
function postPasswordHandler (req, res) {
  const processedReturnURL = processRedirectUrl(req.body.returnURL)
  const submittedPassword = req.body.password
  const validPasswords = config.getConfig().passwords
  if (validPasswords.some(password => submittedPassword === password)) {
    setAuthenticationCookie(res, submittedPassword, 30)
    res.redirect(processedReturnURL)
  } else {
    res.redirect('/manage-prototype/password?error=wrong-password&returnURL=' + encodeURIComponent(processedReturnURL))
  }
}
