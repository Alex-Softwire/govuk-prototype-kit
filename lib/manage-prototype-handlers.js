...
function getPasswordHandler (req, res) {
  ...
  const queryStringPassword = req.query.prototypePassword
  if (queryStringPassword) {
    checkPasswordAndRedirect(res, queryStringPassword, returnURL)
  }
...
function postPasswordHandler (req, res) {
  ...
  checkPasswordAndRedirect(res, submittedPassword, returnURL)
}

function checkPasswordAndRedirect (res, submittedPassword, returnURL) {
  const passwords = config.getConfig().passwords
  const processedRedirectUrl = (!returnURL || returnURL.startsWith('/manage-prototype/password')) ? '/' : returnURL
